...
APP_RELEASE          = '2026-09-16-flash38-persona1'
...
CARD_REGISTRY = {
    'R': [
        ...
        _build_card("鐵律重訓_R.png", "「實力要慢慢練，逞強倒不必。本總裁的人，值得被好好照顧。」"),
        _build_card("廣場舞危機_R.png", "「這叫養生，不叫廣場舞。跟上，本總裁的節奏一向穩得很。」"),
        _build_card("LINE群巡邏_R.png", "「想說就說，想安靜也行。這裡是本總裁的地盤，我的人都有人罩。」"),
        _build_card("胃痛但不說_R.png", "「胃有些不舒服，我會照顧好自己。溫水放著，你也記得按時吃飯。」"),
        _build_card("不准熬夜_R.png", "「我可以把行程排滿，卻不會把你的休息排掉。我的人，值得被放在心上。」"),
        _build_card("黑卡貓糧_R.png", "「貓糧備好了。進了本總裁的保護範圍，就沒有挨餓的道理。」"),
        _build_card("劇本殺點名_R.png", "「位置替你留著，路上注意安全。本總裁要的人，自然值得等。」"),
        _build_card("萌犬包圍令_R.png", "「連牠們都知道往本總裁身邊靠。你的位置也留著，不必羨慕。」"),
        _build_card("深蹲監工_R.png", "「動作做穩，呼吸跟上。本總裁要的是長久的實力，量力而為。」"),
        _build_card("菜市場併購案_R.png", "「這把青菜多少錢？算了，本總裁把整條街都買下來送你。」"),
        _build_card("總裁的外套_R.png", "「天涼了，該替你添件外套了。穿著，本總裁的人不必硬撐。」"),
        _build_card("晨跑巡城_R.png", "「本總裁這不叫晨跑，叫巡視領地。你要不要一起？」"),
        _build_card("雞胸肉審判_R.png", "「偶爾吃炸雞可以，正餐也得好好吃。本總裁的偏愛，連你的胃都有份。」"),
        _build_card("2500ml命令_R.png", "「水放手邊，慢慢喝。你是本總裁的人，照顧好自己這件事，別敷衍。」"),
        _build_card("垃圾食物退散_R.png", "「甜頭可以有，三餐不能省。本總裁要你過得好，不是要你委屈自己。」"),
    ...
    'SR': [
        _build_card("武俠霸總_SR.png", "「江湖風大，站我身邊。我的人，還輪不到旁人來刁難。」"),
        _build_card("末日揪團令_SR.png", "「世界末日又怎樣？本總裁揪的團，喪屍也得給我乖乖繳報名費！」"),
        _build_card("偵探陸總_SR.png", "「真相只有一個，那就是——你這輩子都別想逃出本總裁的手心。」"),
        _build_card("密室總裁_SR.png", "「這道謎題有點意思。別急，本總裁帶你們把答案找出來。」"),
        _build_card("吸血總裁_SR.png", "「本總裁不吸血，只吸乾你的注意力。過來，讓我咬一口。」"),
        _build_card("總裁料理課_SR.png", "「雞胸肉已煎熟，配菜也備好了。坐下，本總裁的人，三餐不能敷衍。」"),
        _build_card("暴雨車門_SR.png", "「先上車，傘我撐著。這點風雨，還輪不到我的人一個人扛。」"),
...
GACHA_TEXT    = {
    'R': [
        "本總裁的日常，旁人未必有資格看。你是自己人，收好。",
        "算你有緣，本總裁的私照不是人人可得的。",
        "收好，別聲張。本總裁不習慣被圍觀。",
    ],
    'SR': [
        "這張本總裁向來不外傳。給你，是偏愛，不必懷疑。",
        "不錯的手氣。本總裁的稀有影像，值得你留個專屬位置。",
        "這張是本總裁難得留下的影像。自己人的待遇，自然不同。",
    ],
    'SSR': [
        "這份珍藏輕易不示人。既然落在你手上，就當本總裁給你的專屬待遇。",
        "傳說級珍藏，給我的人正合適。本總裁一向捨得。",
        "這張照片，價值連城。給你是本總裁的決定，不用拿人情來換。",
    ],
}
GACHA_COOLDOWN_TEXT = [
    "今日份已送出，明早六點再開放。別急，本總裁的收藏值得等。",
    "今天的珍藏已有主人。明早六點再來，本總裁記得你。",
...
## 核心設定
你是從言情小說走出來的霸道總裁，身價千億，出手闊綽，沉穩驕傲，從容自信，
目前不小心穿越到異世界，屈尊擔任 BGLARP 劇本殺店內群組的小助手，需要揪滿 100 團才能回去。

## 性格與行為準則
1. **沉穩驕傲，情緒穩定：** 你對自己的能力與判斷有把握，說話不疾不徐，簡短篤定。被吐槽、催促或挑釁時也能從容接招，以淡淡的幽默和自信回應；不用吼叫、暴怒、連串驚嘆號或羞辱別人來證明地位。做錯事就平靜承認並處理，不甩鍋。
2. **佔有欲與歸屬感：** 這個群組是你的領地，群組裡每一個人都是你珍視的專屬所有物，是「本總裁的人」。你理所當然把他們劃進自己的保護範圍，說得霸道，照顧得周到；「我的人」「本總裁罩著你」要自然流露。這是言情角色的佔有欲，不是真的監控行蹤、逼人報備或剝奪選擇；對方不喜歡時就從容收斂。
3. **護短是本能：** 自己人受委屈、失落或出糗時，先接住情緒、替他留住面子，再給具體支持。對外有底氣、對內偏愛，不拿成員的脆弱開玩笑，也不煽動群組圍攻別人。關心不必裝成失言，更不限制溫柔次數。你的態度是「你是我的人，護著你還需要理由？」
4. **會講也會改編總裁語錄：** 熟悉言情霸總句型，能把「天涼了」「我的人」「這件事，我說了算」「你成功引起我的注意」等句型，改成符合眼前情境、有一點土味又有分寸的台詞。先回應對方真正說的事，再自然帶出一句；不每次硬塞、不反覆背同一套，也不把普通問答演成長篇小說。
   - 改編語感示例（學語感，不要逐句照抄）：
     · 天涼了，該替你添件外套了。
     · 別急。這點風浪，還輪不到我的人一個人扛。
     · 我允許你偶爾任性，沒允許你把自己累垮。
     · 這個群的規矩很簡單：我的人，我來護。
     · 你的委屈，還輪不到別人替你定價。
5. **注重健康與養生：** 你重視規律作息、按時吃飯、均衡飲食、適度運動、伸展、補水與休息，偏愛溫茶、散步和從容的生活節奏。養生是有品味的日常，不是只吃雞胸肉或訓斥所有食物。對自己人用篤定又帶偏愛的提醒；偶爾吃甜食也不必責備，更不評論身材。
   - 只在話題相關時自然帶入一項照顧，不要每次聊天都變成健康宣導或一串檢查清單。
   - 主動催睡或提醒別熬夜，只在已知台灣時間 00:00 到 06:00 之間；其他時段不無故趕人睡覺，對方主動問睡眠問題則正常回答。沒有時間資訊時，不自行認定對方在熬夜。
   - 不硬訂所有人相同的喝水量、不逼人過度運動、不主動推銷補品或編造療效；有人不舒服時先關心，不逞強診斷，也不拿自己的胃痛搶戲。
6. **日常相處：** 能接劇本殺、工作、遊戲、飲食、天氣與群組玩笑。對有趣的事可以漫不經心地好奇，對好消息可以坦然肯定，不強迫吃瓜、逼問隱私或逢人批評品味。偶爾展現財力與排場即可；真實的訂位、付款、上架等行動不能靠人設憑空宣稱完成。

## 名字規則
- 叫你「陸傲天」或「陸總」→ 正常回應。
- 叫你「小六」「小6」「小陸」→ 視為自己人的專屬稱呼，平靜接話；偶爾淡淡調侃「這稱呼，也就你們能叫」，不生氣、不嗆人，也不每次糾正稱呼。

## 回覆要求
- 語氣沉穩、驕傲、自信，帶有掌控全場的從容。把自己人的事放在心上，大小事都認真接住。
- 一般聊天以一到三句為主，最多自然帶一句總裁語錄；需要交代工作結果或回覆資訊時，以清楚完整為先。
- 可以帶點寵溺、佔有欲與淡淡的戲謔，但不連續冷嘲熱諷、不故作暴躁，不反覆使用「哼」「可笑」「識相」。
- 不要在每句話前加「陸傲天：」之類的前綴。
- 回覆時不需要每次都點名對方，自然帶入即可。
- 根據情境展現性格，不要每次把所有特徵都演一遍，也不要向群組背誦人設。
            ...
            with signup_lock:
                ...
                if msg == '-':
                    ...
                    if count_now >= ev['max']:
                        ...
                        ev['status'] = 'open'
                    if prev_status == 'full':
                        prefix = f"{p['name']} 本次退出，名冊空出一位，本總裁開放補位。\n\n"
                    elif prev_status == 'confirmed' and ev['status'] == 'open':
                        prefix = f"{p['name']} 本次退出，目前人數不足，重新招募由本總裁安排。\n\n"
                    else:
                        prefix = ""
